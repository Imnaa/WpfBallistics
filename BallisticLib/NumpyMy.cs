...
namespace BallisticLib
{
    static class NumpyMy
    {
        ...
        {
            switch (algoritm.ToLower())
            {
                case "lagrange":
                    return Lagrange(h_f, h_ar.ToArray(), ro_ar.ToArray());
                case "newton":
                    return Newton(h_f, h_ar.ToArray(), ro_ar.ToArray());
                case "linear":
                    return Linear(h_f, h_ar.ToArray(), ro_ar.ToArray());
                default:
                    throw new ArgumentException("Метод выбран неверный");
