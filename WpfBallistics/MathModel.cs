...
namespace WpfBallistics
{
    class MathModel
    {
        ...
        public bool Start(Ballistic ballistic, Dictionary<string, double> calcParams, double angle)
        {
            if (calcParams.Count != 6)
            {
                throw new ArgumentException("Неверное количество параметров интегрирования для вычесления!");
                //return isOk;
            }
            ...
            // Calc params
            dt_f = Convert.ToDouble(calcParams["tbStepIntegr"]);
            tk_f = Convert.ToDouble(calcParams["tbTimeCalc"]);
